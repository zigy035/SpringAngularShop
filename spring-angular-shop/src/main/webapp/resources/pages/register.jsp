...
<form:form commandName="registerForm" action="${registerURL}" method="post" ng-app="addressApp" ng-controller="addressController">
		...
		<p>
			...
			<form:select path="title">
				...
				<c:forEach items="${titles}" var="title">
					<c:choose>
						<c:when test="${title.code eq registerForm.title}">
							<option value="${title.code}" selected="selected">
								${title.name}
							</option>
						</c:when>
						<c:otherwise>
							<option value="${title.code}">
								${title.name}
							</option>
						</c:otherwise>
					</c:choose>
